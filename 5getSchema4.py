# coding=utf-8
#得到match schema3 match增加Q'
# 5000 為一組，共5000,000條數據, 每組前五個為答案

#QQ' 3個為training, 1個為dev, 1個為test
#test 為1000個一組 答案在第一個(0, 1001, 2001...)
#收集所有非正確Q', 從中選n個
#python3 5getSchema4.py  ./data/5000data.xlsx ./data_7_15_Schema4/
            ...
            for j in range(5):
                ...
                if index % 5 == 0 or index % 5 == 1 or index % 5 == 2:
                    ...
                    testCSVW.writerow([
                        0,
                        rmSymbol(data['測試題'][index]),
                        rmSymbol(data['測試題'][index - 1])
                    ])
                    testCSVW.writerow([
                        0,
                        rmSymbol(data['測試題'][index]),
                        rmSymbol(data['測試題'][index - 2])
                    ])
                    testCSVW.writerow([
                        0,
                        rmSymbol(data['測試題'][index]),
                        rmSymbol(data['測試題'][index - 3])
                    ])
                    testCSVW.writerow([
                        0,
                        rmSymbol(data['測試題'][index]),
                        rmSymbol(data['測試題'][index - 4])
                    ])
                    ...
                    for i in range(4995):
                        ...
                        testCSVW.writerow([
                            0,
                            rmSymbol(data['測試題'][index]),
                            rmSymbol(data['測試題'][(index + i + 1) % 5000])
                        ])